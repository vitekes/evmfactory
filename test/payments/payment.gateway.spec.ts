...
describe('PaymentGateway', function () {
  ...
  it('processes ERC-20 payments', async function () {
    ...
    await expect(tx).to.changeTokenBalances(ethers, token, [payer, moduleCaller], [-ERC20_AMOUNT, ERC20_AMOUNT]);
  });
  ...
  it('processes native payments', async function () {
    ...
    await expect(tx).to.changeEtherBalance(ethers, gateway, 0);
  });
