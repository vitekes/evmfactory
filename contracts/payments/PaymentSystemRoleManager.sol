...
/// @notice Централизованный менеджер ролей для платежной системы
/// @dev Управляет ролями для процессоров и других компонентов платежной системы
contract PaymentSystemRoleManager is AccessControl {
    ...
    // Роли процессоров: процессор => роль => аккаунт => статус
    mapping(address => mapping(bytes32 => mapping(address => bool))) public processorRoles;

    // Системные роли: роль => аккаунт => статус
    mapping(bytes32 => mapping(address => bool)) public systemRoles;
    ...
    event SystemRoleGranted(bytes32 indexed role, address indexed account);
    event SystemRoleRevoked(bytes32 indexed role, address indexed account);

    /**
     * @dev Конструктор менеджера ролей
     * @param _core Адрес системы ядра
     */
    constructor(address _core) {
        require(_core != address(0), 'RoleManager: core system is zero address');
    ...
    /**
     * @notice Предоставить роль процессора аккаунту
    ...
    function grantProcessorRole(
        ...
        bytes32 role,
        address account
    ) external onlyRole(ROLE_MANAGER_ADMIN) {
    ...
    /**
     * @notice Отозвать роль процессора у аккаунта
     * @param processor Адрес процессора
     * @param role Идентификатор роли
     * @param account Адрес аккаунта
     */
    ...
    /**
     * @notice Предоставить системную роль аккаунту
     * @param role Идентификатор роли
     * @param account Адрес аккаунта
     */
    function grantSystemRole(bytes32 role, address account) external onlyRole(ROLE_MANAGER_ADMIN) {
        require(account != address(0), 'RoleManager: account is zero address');

        systemRoles[role][account] = true;

        emit SystemRoleGranted(role, account);
    }

    /**
     * @notice Отозвать системную роль у аккаунта
     * @param role Идентификатор роли
     * @param account Адрес аккаунта
     */
    function revokeSystemRole(bytes32 role, address account) external onlyRole(ROLE_MANAGER_ADMIN) {
        require(account != address(0), 'RoleManager: account is zero address');

        systemRoles[role][account] = false;

        emit SystemRoleRevoked(role, account);
    }

    /**
     * @notice Проверить, имеет ли аккаунт роль процессора
     * @param processor Адрес процессора
     * @param role Идентификатор роли
     * @param account Адрес аккаунта
     * @return Имеет ли аккаунт роль
     */
    ...
    /**
     * @notice Проверить, имеет ли аккаунт системную роль
     * @param role Идентификатор роли
     * @param account Адрес аккаунта
     * @return Имеет ли аккаунт роль
     */
    function hasSystemRole(bytes32 role, address account) external view returns (bool) {
        return systemRoles[role][account];
    }

    /**
     * @notice Проверить, имеет ли аккаунт роль в ядре системы
     * @param role Идентификатор роли
     * @param account Адрес аккаунта
     * @return Имеет ли аккаунт роль
     */
    function hasCoreRole(bytes32 role, address account) external view returns (bool) {
