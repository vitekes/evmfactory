...
## Создание планов
- **Прямой вызов `createPlan`.** Автор сам инициирует транзакцию `PlanManager.createPlan` со своего адреса (он же `plan.merchant`). Подпись и выдача `AUTHOR_ROLE` не требуются, если `msg.sender` совпадает с мерчантом.
- **Ленивое создание.** Когда нужно регистрировать план во время первой подписки, автор подписывает структуру `SignatureLib.Plan` (EIP-712) один раз и отдаёт подпись фронту/бэкенду. `SubscriptionManager.subscribe` пробрасывает `sigMerchant` в `PlanManager.createPlan`; без подписи вызов ревертится, так что пользователь не сможет занизить цену.

## Проверка цены
- Каждое `subscribe` делает `IPlanManager.getPlan(planHash)` и сравнивает платёж (`msg.value` или переведённый ERC-20) с `PlanData.price`. Несоответствие сразу приводит к `InvalidAmount/InsufficientBalance`.
- Бэкенд может кэшировать цены и отбрасывать устаревшие тарифы, но последняя линия обороны находится в контракте. Фронт должен отображать данные из on-chain событий `PlanCreated/PlanStatusChanged` или `getPlan`.
