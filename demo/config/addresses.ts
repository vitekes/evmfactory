...
import fs from 'fs';
import { ethers, ignition } from '../../hardhat-connection';
...
function readEnvAddresses(): DemoAddresses | null {
  ...
  const optionalMonetaryCash = process.env.DEMO_MONETARY_CASH;
  ...
  return {
    ...
    monetaryCash: optionalMonetaryCash || undefined,
  };
}
...
async function deployWithIgnition(): Promise<DemoAddresses> {
  const deployment = await ignition.deploy(DeploymentModule, {
    deploymentId: 'demo',
  });
  ...
  return {
    ...
    monetaryCash:
      'monetaryCash' in deployment
        ? await (deployment as Record<string, any>).monetaryCash.getAddress()
        : undefined,
  };
}
